...
const SHEET_NAMES = {
  ...
  SETTINGS: 'SETTINGS',
  AREA_WEIGHTS: ['E', 'X', 'A', 'M', '_', 'C', 'O', 'N', 'F', 'I', 'G'].join(''),
...
// ============================================================================
// ADVANCED PLANNER & SCHEDULER UTILITIES
// ============================================================================
...
function determineHorizonDays(referenceDate, areaWeightConfig) {
  const today = new Date(referenceDate || new Date());
  today.setHours(0, 0, 0, 0);
  let horizon = 42;
  if (Array.isArray(areaWeightConfig)) {
    areaWeightConfig.forEach(row => {
      if (!row) return;
      const dateKey = ['data', 'Prova'].join('');
      if (!row[dateKey]) return;
      const targetDate = parseSheetDate(row[dateKey]);
...
function gatherReviewCandidates(settings, referenceDate) {
  ...
  const modelData = readSheetData(SHEET_NAMES.MODEL);
  const areaWeightConfig = readSheetData(SHEET_NAMES.AREA_WEIGHTS);
  ...
  hoje.setHours(0, 0, 0, 0);
  const horizonDays = determineHorizonDays(hoje, areaWeightConfig);
...
function apiPlanDayBudget(params) {
  try {
    ...
    const enrichedTargets = plan.targets.map(target => {
      ...
      const guidePlan = computeStudyGuidePlanV2(target.alvo, allocMinutes, prefsForGuide, planningSettings, dataCache, logRows, revisaoRows, areaWeightConfig, todayDate);
      if (guidePlan) {
        if (guidePlan.status) {
...
function apiStudyGuidePlanV2(params) {
  try {
    ...
    const plan = computeStudyGuidePlanV2(alvoRaw, budgetMin, prefs, planningSettings, data, logRows, revisaoRows, areaWeightConfig, today);
    if (!plan) {
...
function computeStudyGuidePlanV2(alvoRaw, budgetMin, prefs, settings, data, logRows, revisaoRows, areaWeightConfig, today) {
  if (!alvoRaw || !settings || !data) return null;
  const alvoKey = alvoRaw.toString().trim();
  if (!alvoKey) return null;
  ...
  if (status === 'NOVO') {
    ...
    const horizonDays = determineHorizonDays(todayDate, areaWeightConfig);
...
function apiLogReviewOutcome(payload) {
  ...
  try {
    ...
    const areaWeightConfig = readSheetData(SHEET_NAMES.AREA_WEIGHTS);
    const horizonDays = determineHorizonDays(hojeSemHora, areaWeightConfig);
...
function apiRecompute() {
  ...
  try {
    ...
    const spacedSheet = getOrCreateSheet(SHEET_NAMES.SPACED, HEADERS.SPACED);
    const areaWeightConfig = readSheetData(SHEET_NAMES.AREA_WEIGHTS);
    const horizonDays = determineHorizonDays(new Date(), areaWeightConfig);
